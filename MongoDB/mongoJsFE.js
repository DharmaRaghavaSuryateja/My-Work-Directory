...
class AppModel {
  ...
  async addItem(todoItem) {
    ...
    try {
      var result = await fetch("http://localhost:8080/addlist", {
        method: "POST",
        headers: {
  ...
  async updatestatus(ide) {
    try {
      await fetch(`http://localhost:8080/update/${ide._id}`, {
        method: "POST",
        headers: {
          "Content-type": "application/json",
        },
 ...
 ButtonBorder() {
  ...
  async render() {
    ...
    try {
      var response = await fetch(`http://localhost:8080/hey/${app.filter}`);
    } catch (error) {
      console.log(error);
    }
